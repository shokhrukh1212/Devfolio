...
export function TerminalTheme({ data }: { data: ProfileResponse }) {
  const [typedText, setTypedText] = useState("");
  const fullText = `> Initializing profile for ${data.username}...\n> Loading modules... DONE\n> Rendering bio...`;
    ...
    <div className="min-h-screen bg-[#0d1117] text-[#58a6ff] font-mono p-6 md:p-12 overflow-x-hidden">
      <div className="max-w-4xl mx-auto border border-[#30363d] rounded-lg bg-[#010409] shadow-2xl overflow-hidden">
        ...
        <div className="bg-[#161b22] px-4 py-2 flex items-center gap-2 border-b border-[#30363d]">
          ...
          <div className="ml-4 text-xs text-[#8b949e]">user@{data.username}: ~</div>
        </div>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <div className="flex gap-6 mt-6 text-sm">
                {data.githubUrl && (
                  <a href={data.githubUrl} className="text-[#58a6ff] hover:underline underline-offset-4">
                    github_url: "{data.githubUrl}"
                  </a>
                )}
                {data.twitterUrl && (
                  <a href={data.twitterUrl} className="text-[#58a6ff] hover:underline underline-offset-4">
          ...
          >
             ...
             <div className="grid gap-6">
                {data.projects?.filter(p => p.isVisible).map((project) => (
                  <div key={project.id} className="border border-[#30363d] rounded p-4 hover:border-[#58a6ff] transition-colors group bg-[#0d1117]">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {project.techStack?.map(tech => (
                        <span key={tech} className="text-xs text-[#79c0ff] bg-[#388bfd]/10 px-2 py-0.5 rounded border border-[#388bfd]/40">
                          {tech}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="flex gap-4 text-sm">
                       {project.demoUrl && (
                         <a href={project.demoUrl} target="_blank" rel="noreferrer" className="text-[#2ea043] hover:underline">
                           ./view-demo.sh
                         </a>
                       )}
                       {project.githubUrl && (
                         <a href={project.githubUrl} target="_blank" rel="noreferrer" className="text-[#d2a8ff] hover:underline">
                           ./view-source.sh
                         </a>
                       )}
                    </div>
